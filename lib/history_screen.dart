...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Widget _buildHistoryItem(Flower flower) {
    ...
    // Decode รูปต้นฉบับ (imageBase64 = รูปที่ถ่าย)
  ...
  /// แสดง popup รูปที่มีกรอบ detection (สำหรับ API version)
  void _showDetectedImageDialog(Flower flower) {
    // Decode รูปที่ detect แล้ว (มีกรอบ - ถ้ามี)
    ...
    // ถ้าไม่มีรูปที่มีกรอบ ให้ใช้รูปต้นฉบับแทน
    if (detectedImageBytes == null &&
        flower.imageBase64 != null &&
        flower.imageBase64!.isNotEmpty) {
      try {
        detectedImageBytes = base64Decode(flower.imageBase64!);
      } catch (e) {
        print('Error decoding original image: $e');
      }
    }
                      ...
                      if (flower.confidence != null) ...[
                        ...
                        Row(
                          children: [
                            ...
                            Text(
                              'ความมั่นใจ: ${flower.confidence!.toStringAsFixed(0)}%',
                              style: const TextStyle(
                                fontFamily: 'Kanit',
                                fontSize: 14,
                                color: Colors.white,
